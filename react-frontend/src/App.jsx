...
function RecentAlerts({ alerts }) {
  return (<div><h2>Recent Alerts</h2><div style={styles.grid}>{alerts.map(a => (<div key={a.id} style={styles.card} onMouseEnter={e => e.currentTarget.style.transform = styles.cardHover.transform} onMouseLeave={e => e.currentTarget.style.transform = 'none'}><h3>{a.title}</h3><p>{a.time}</p></div>))}</div></div>);
}
...
export default function App() {
  ...
  const pages = {
    'Recent Alerts': () => <RecentAlerts alerts={error ? dummyAlerts : alerts} />,    
